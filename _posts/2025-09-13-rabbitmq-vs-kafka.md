...
tags:
  ...
  - 메시징
  - 비동기
description: 'RabbitMQ와 Kafka의 차이점과 각 시스템의 특징을 비교하여, 적합한 사용 사례를 제시합니다.'
image:
...
현재 팀에서, RabbitMQ 를 통한 비동기 및 메시징 애플리케이션 에서 개발을 하고 있다.
흔히, 거대한 기업들은 Kafka 를 쓰는데 왜 우리 팀은 RabbitMQ 를 쓰는지 궁금했고 그래서 찾아보고 나름대로의 답을 찾은거 같다.
...
RabbitMQ는 **AMQP(Advanced Message Queuing Protocol)** 를 구현한 메시지 브로커  ( MQTT 프로토콜도 제공 )
...
*   **Producer (생산자):** 메시지를 생성하고 **Exchange**로 전송
*   **Exchange (교환기):** Producer로부터 받은 메시지를 어떤 **Queue**로 보낼지 결정하는 라우팅 규칙의 집합입니다. Exchange는 메시지 자체를 저장하지 않습니다
...
=> 우체국(브로커)이 모든 편지 분류 및 주소를 알려주고, 집배원(소비자) 가 자기 구역 할당된 편지를 배달하는 시스템
...
*   **Offset (오프셋):** 파티션 내에서 각 메시지가 갖는 고유한 순번(ID)입니다. 소비자는 오프셋을 통해 어디까지 메시지를 읽었는지 추적하고 제어
	* **Commit Offset** : 컨슈머가 `여기까지 오프셋을 처리했다` 를 확인하는 오프셋
	* **Current Offset** : 컨슈머가 `어디까지 메시지를 읽었는지` 를 확인하는 오프셋
...
- 읽을 위치 직접 관리 : 컨슈머가 자신이 토픽의 어떤 파티션, 몇번째 메시지 까지 읽었는지 스스로 기록 & 관리
