...
---
이번 내용 부터는 fzf 를 통해 어떻게 함수들을 연결할 수 있는지 알아보자.

> 내용이 길고, 다수 함수만 설명하는 내용일 수도 있다..
> 말하고 싶은 요지는 이와같이 개인 및 팀이 느끼는 불편함을 화면 기반으로 해결할 수 있는걸 보여주고 싶은 것이다.
...
- Mode 1 이면, fzf 를 사용하지 않고 바로 다운로드를 시도한다.
...
다소 길어 보일 수 있는데 위 `지라 이슈 조회` 와 `stash 목록 관리` 의 심화 버전이다.
