...
## 📝 초안 내용 (Markdown)
> 프로젝트에서 사용하는 Jakcson 어노테이션 및 기능들을 정리한 내용이다.
...
만 쓰기에는 엄청나게 아쉬운 매우 매력적인 라이브러리다.

아무리 시대가 달라져도 웹요청의 근본은 JSON 이기 때문이다. 그리고, Java 에서 직렬/역직렬화는 생각보다 까다롭다. ( `implements Serializable`, `byte 단위 변환` )
이 모든걸 최대한 우리 편의에 맞게 맞춰주는게 Jackson 과 ObjectMapper 이다.

아래는 어노테이션들과 Jackson 을 더 깊게 써보는 방법들에 대해 가볍게 다루어보았다.
...
주로, 이 2개는 테스트 할 때 사용했다.

readValue 는 정말 요청 자체를 직렬화 한다. (URL, InputStream, Byte, String 등등등)

readValue 를 통해 ENUM 값에 존재하지 않는 값을 넣을때 어떻게 직렬화 되는지 등을 테스트 할 수 있다.
( 즉, 일반적인 우리 자바 구조로는 존재하지 않는 요소들을 테스트 할 때 용이 )

convertValue 는 Java 객체를 직렬화 한다.
...
이와같을때, DB에 존재하는 값은 가지고 오나 저장할 때는 값이 저장되지 않게 사용가능하다.
...
`@JsonView(DtoView.Client.class)` 를 선언하면 Client 는 역직렬화 되지 않는다.
...
-> 매우 자주 사용한다. ⭐️ ( DTO 의 변화에 엄격하게 반응하기 어려울 수도 있음. - 수정 되거나, 삭제 되거나, 꼭 있어도 되지 않는 데이터 등등 )
...
- allowGetters : 읽기 전용, 출력(직렬화)은 허용하나 입력(역직렬화)는 안 됨
- allowSetters : 쓰기 전용, 입력(역직렬화)은 허용하나 출력(직렬화)는 안 됨

해당 부분은 다소 사용하기 어렵다. DB 에서 값을 가져오되, DB 에 값은 저장되지 않게 하고 싶다면?
1. 역직렬화를 통해 객체를 만든다.
2. 객체를 JSON 으로 직렬화 해 HTTP 로 반환한다.

DB 에 빈 값을 저장하기 싫어서 직렬화를 막는다면? -> HTTP 로 직렬화 할 때도 포함되지 않는다.
...
- JsonFormat : Jackson 제공 어노테이션, 패턴에 맞게 변환, 타임존을 지정하여 특정 시간대로 변환 가능 ( `"2025-06-16T14:36:36"` 와 같이 변환 )

두개를 같이 지정하여, 스프링 요청으로 오든 JSON 요청,응답으로 오든 일관되게 처리 가능하다.

### JsonTypeInfo / JsonSubTypes

다형성 구조를 Jackson 으로 직렬 / 역직렬화 해주는 어노테이션이다.
흔히 Java 의 꽃은 Interface 라고 하지 않던가.
하지만 Jakckson 이 인터페이스의 정보만 있으면 어떻게 직/역직렬화를 해야 하는지 알 수 없다.
...
Jackson 은 직렬화 / 역직렬화 할 때 위 수많은 어노테이션들을 읽어서 결정한다.
...
사실, 프로젝트를 하면서 직렬화 / 역직렬화 마스터가 되어가는거 같다 ㅋㅋ
