...
function getEnvVars() {
  ...
  if (!repoFullName || !issueNumber || !issueTitle || !issueBodyRaw || !token || !openaiApiKey) {
    core.setFailed('필요한 환경 변수가 누락되었습니다.')
    process.exit(1)
  }

  core.info('✅ 환경 변수 모두 읽어왔습니다.')
  return { repoFullName, issueNumber, issueTitle, issueBodyRaw, token, openaiApiKey }
}
...
function generateSlugAndTitle(issueTitle) {
  ...
  let slug = slugify(cleanedTitle, { lower: true, strict: false, locale: 'ko' })
...
async function generateAndDownloadImage(openaiApiKey, cleanedTitle, datePrefix, slug) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`이미지 생성 오류: ${response.status} ${response.statusText} - ${errText}`)
  }
 ...
 */
async function commitAndCreatePR(octokit, owner, repo, baseCommitSha, branchName, mdFilePath, fullMarkdown, imagePath) {
  // 1) Markdown Blob 생성
...
async function run() {
  try {
    // 1) 환경 변수 읽기
    const { repoFullName, issueNumber, issueTitle, issueBodyRaw, token, openaiApiKey } = getEnvVars()
    ...
    const baseCommitSha = await createBranch(octokit, owner, repo, defaultBranch, branchName)
    ...
    // 8) 썸네일 생성 및 다운로드
    const imagePathForFrontmatter = await generateAndDownloadImage(openaiApiKey, cleanedTitle, datePrefix, slug)

    // 9) Frontmatter 작성
    const frontmatter = composeFrontmatter(cleanedTitle, tags, description, imagePathForFrontmatter, now)

    // 10) fullMarkdown 구성
    const fullMarkdown = frontmatter + issueBodyTrimmed + '\n'

    // 11) Commit & PR 생성
    await commitAndCreatePR(octokit, owner, repo, baseCommitSha, branchName, mdFilePath, fullMarkdown, imagePathForFrontmatter)
