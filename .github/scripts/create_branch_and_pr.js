...
function getEnvVars() {
  ...
  let decodedTitle
  try {
    decodedTitle = decodeURIComponent(issueTitle)
  } catch {
    decodedTitle = issueTitle
  }
...
async function ensureBranch(octokit, owner, repo, defaultBranch, branchName) {
  try {
    ...
    await octokit.git.createRef(
      {owner, repo, ref: `refs/heads/${branchName}`, sha: baseCommitSha})
...
async function commitAndCreatePR(octokit, owner, repo, baseCommitSha,
  branchName, title, mdFilePath, imagePath, defaultBranch) {
  ...
  try {
    ...
    if (e.status === 422 && String(e.message).includes(
...
async function writeJobSummary({
  ...
  .addHeading('🧩 태그', 3)
  .addRaw(frontmatter.tags && frontmatter.tags.length ? frontmatter.tags.map(
    t => `\`${t}\``).join(' ') : '(없음)')
  .addHeading('📝 설명', 3)
...
async function run() {
  ...
  try {
    ...
    execSync(
      `node .github/scripts/generate_metadata_from_file.js --file=${tempFilePath}`,
    ...
    fs.writeFileSync(tempFilePath,
      matter.stringify(matter(updatedContent).content, frontmatter))
    ...
    core.setOutput('file_path', finalFilePath)
